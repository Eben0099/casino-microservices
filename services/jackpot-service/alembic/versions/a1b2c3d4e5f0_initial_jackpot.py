"""Initial jackpot tables (pots, contributions, wins)

Mirrors ticket-service c4d8f1a96bbf with key adaptation:
  kiosk_id UUID -> kiosk_code String(20)
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        # ADAPTATION: kiosk_code String(20) au lieu de kiosk_id UUID
        sa.Column("kiosk_code", sa.String(length=20), nullable=True),
    ...
    op.create_unique_constraint(
        ...
        "jackpot_pots",
        # ADAPTATION: kiosk_code au lieu de kiosk_id
        ["scope", "game_id", "kiosk_code", "tier"],
    )
    op.create_index("ix_jackpot_pots_scope_game", "jackpot_pots", ["scope", "game_id"])
    op.create_index("ix_jackpot_pots_kiosk_code", "jackpot_pots", ["kiosk_code"])
    ...
    op.create_table(
        ...
        sa.Column(
            "pot_id",
            sa.UUID(),
            sa.ForeignKey("jackpot_pots.id", ondelete="CASCADE"),
            nullable=False,
        ),
        # ticket_id est un String (UUID cross-service sans FK)
    ...
    op.create_index(
        "ix_jackpot_contributions_pot_id", "jackpot_contributions", ["pot_id"]
    )
    op.create_index(
        "ix_jackpot_contributions_ticket_id", "jackpot_contributions", ["ticket_id"]
    )
    op.create_unique_constraint(
    ...
    op.create_table(
        ...
        sa.Column("cycle_number", sa.Integer(), nullable=False),
        sa.Column("trigger_ticket_id", sa.String(length=36), nullable=False),
        sa.Column("winner_ticket_id", sa.String(length=36), nullable=False),
        sa.Column("winner_agent_id", sa.String(length=36), nullable=False),
        ...
        sa.Column(
            "carryover", sa.BigInteger(), nullable=False, server_default="0"
        ),
        sa.Column(
            "paid_at", sa.DateTime(timezone=True), server_default=sa.text("now()")
        ),
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "uq_pot_ticket_contribution", "jackpot_contributions", type_="unique"
    )
    op.drop_index(
        "ix_jackpot_contributions_ticket_id", table_name="jackpot_contributions"
    )
    op.drop_index(
        "ix_jackpot_contributions_pot_id", table_name="jackpot_contributions"
    )
    op.drop_table("jackpot_contributions")

    op.drop_index("ix_jackpot_pots_kiosk_code", table_name="jackpot_pots")
    ...
    for enum_name in (
        "resetmode", "contributionmode", "winnermode", "jackpottier", "jackpotscope"
    ):
