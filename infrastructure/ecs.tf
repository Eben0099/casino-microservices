...
locals {
  microservices = {
    "agent-service" = {
      ...
      env = [
        { name = "DATABASE_URL", value = "postgresql+asyncpg://postgres:SuperSecretPassword2026@${aws_db_instance.postgres.address}:5432/casino_db" },
        { name = "ADMIN_API_KEY", value = "CleSuperSecreteBackoffice2026" },
